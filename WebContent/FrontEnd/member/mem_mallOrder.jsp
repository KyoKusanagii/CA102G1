...
<% 
	...
	List<MallOrderVO> list = mallorderSvc.getBuyerMallOrder(mem_no);
		...
		<div class="row" style="font-family:微軟正黑體; font-weight:bold">
			...
			<div class="container col-md-8" >
				...
				<ul class="nav nav-tabs">
				  <li class="active"><a href="#">商城訂單</a></li>
				  <li><a href="mem_fbOrder.jsp">FB訂單</a></li>	 
				</ul>
				 		...
				 		<table class="table">
					 		<tr>
					 			<td>
					 				<div class="text-align-center" style="Display:inline-block;float: left; margin-left: 20px;">
					 					<img src="data:image/*;base64,${MemService.findByPK(MallOrderService.getOrderSeller(mallorderVO.mall_order_no)).profilepicEncoded}" class="img-responsive img-circle" style="height:50px; width:50px">
					 				
					 				</div>
					 				<div style="Display:inline-block;float: left; margin-left: 20px; margin-top:15px">
					 					<font>${MemService.findByPK(MallOrderService.getOrderSeller(mallorderVO.mall_order_no)).mem_id} (${MemService.findByPK(MallOrderService.getOrderSeller(mallorderVO.mall_order_no)).mem_name})</font>
					 				</div>
					 				<div class="text-right" style="margin-left:80%; margin-top:15px; margin-right:20px" >
					 					${mallorderVO.mall_pay_status}
					 				</div>
					 		...
					 		<tr>
					 			<td>
					 				...
					 				<c:forEach var="buyerorderVO" items="${MallOrderService.getAllOrderItem(mallorderVO.mall_order_no)}">
					 				<div class="row">
						 				<div style="Display:inline-block;float: left; margin-left: 20px;">
						 					<img src="data:image/*;base64,${ItempicService.getThumbnail(buyerorderVO.item_no).encoded}" class="img-responsive " style="height:80px; width:80px">
					 		...
					 		<tr>
					 			<td>
					 				<div style="float:right;">
					 					應付總額  : $ ${mallorderVO.mall_order_prc}
					 				</div>
					 			</td>
					 		</tr>
					 		<tr>
					 			<td>
					 				<c:if test="${mallorderVO.mall_order_status eq '進行中'}">
						 				<div style="Display:inline-block;float: left; margin-left: 20px; color:blue;" class="mall_order_status">
						 					${mallorderVO.mall_order_status}
						 				</div>
					 				</c:if>
					 				<c:if test="${mallorderVO.mall_order_status eq '已完成'}">
						 				<div style="Display:inline-block;float: left; margin-left: 20px; color:green;" class="mall_order_status">
						 					${mallorderVO.mall_order_status}
						 				</div>
					 				</c:if>
					 				<c:if test="${mallorderVO.mall_order_status eq '取消'}">
						 				<div style="Display:inline-block;float: left; margin-left: 20px; color:red;" class="mall_order_status">
						 					${mallorderVO.mall_order_status}
						 				</div>
					 				</c:if>
					 				...
					 				<div style="Display:inline-block; float: right;">
					 					<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${mallorderVO.mall_order_time}" />
					 				</div>
					 			</td>
					 		</tr>
					 		<tr>
						 			...
						 			<td class="text-right">
						 				...
						 				<c:if test="${!(mallorderVO.mall_order_status eq '已完成' || mallorderVO.mall_order_status eq '取消' || mallorderVO.mall_pay_status eq '已付款')}"> 
							 				<div style="Display:inline-block;">
								 				<button class="btn btn-danger cancel" id="${mallorderVO.mall_order_no}" >取消訂單</button>
							 				</div>
						 				</c:if>
						 			
						 				
						 				<div style="Display:inline-block;">
							 				<form method="post" action = "<%=request.getContextPath()%>/FrontEnd/mallorder/mallOrder.do">
							 					<button type="submit" class="btn btn-warning">查看詳情</button>
							 					<input type="hidden" name="action" value="showOneMallOrder">
							 					<input type="hidden" name="mall_order_no" value="${mallorderVO.mall_order_no}">
							 			    </form>
						 				</div>
						 			</td>
	...
	$(document).ready(function () {
	      ...
	      $(".cancel").click(function(){
	    	  if(window.confirm('確定要取消此筆訂單?')){
	 	         ...
	 	         var mall_order_no = $(this).attr("id");
	 	         var mall_buyer_no = "${memVO.mem_no}";
	    $.ajax({
	     ...
	     data : {
	        ...
	        mall_buyer_no:mall_buyer_no 
	     },
	    
	    });
	    alert("已成功取消此筆，並通知賣家家");
	      $(this).hide();
	      $(this).parent().parent().parent().prev().children().children("div.mall_order_status").text("取消").css("color","red");
	    	 }
	   });
	  });
	</script>
