...
<body>
	...
	<div class="col-md-6 col-md-offset-3" style="resize:none; box-shadow: 1px 1px 8px black; margin-top:10px">
		<div class="row">
		...
		<% 
			if(! "取消".equals(mallorderVO.getMall_order_status())){
		%>
		<div class="text-center " id="change-button" style="margin-top:20px; margin-bottom:20px">
			 ...
			 <c:if test="${mallorderVO.mall_pay_status eq '已付款'&& mallorderVO.mall_order_status eq '進行中'}">
			 	<button class="btn btn-primary btn-block" id="confirmReceive">確認收貨</button>
			 </c:if>
			 <c:if test="${mallorderVO.mall_pay_status eq '未付款'}">
			 	<button class="btn btn-success btn-block" id="confirmCheck">結帳確認</button>
			 </c:if>
	...
	<div class="text-center col-md-12" style="margin-top:50px"><a href="<%=request.getContextPath()%>/FrontEnd/member/mem_mallOrder.jsp" class="btn btn-default">返回訂單列表</a></div>
	</div>	
		<!-- footer -->
	...
	$(function () {
	    $('#confirmCheck').on('click',function(){
	     if(window.confirm('確定已結帳?')){
	         var action = "confirmCheck";
	...
	$(function () {
	    $('#confirmReceive').on('click',function(){
	     if(window.confirm('確定收貨嗎?')){
	         var action = "confirmReceive";
	         var mall_order_no = "${mallorderVO.mall_order_no}";
	         var mall_buyer_no = "${memVO.mem_no}";
	      $.ajax({
	       url : "<%= request.getContextPath()%>/FrontEnd/mallorder/mallOrder.do",
