...
<body>
	...
	<div class="col-md-6 col-md-offset-3" style="font-family:微軟正黑體;resize:none; box-shadow: 1px 1px 8px black; margin-top:10px">
		...
		<div>賣家 : ${MemService.findByPK(FbOrderService.getOrderSeller(fborderVO.fb_order_no)).mem_name}</div>
		...
		<% 
			if(! "取消".equals(fborderVO.getFb_order_status())){
		%>
		<div class="text-center " id="change-button" style="margin-top:20px; margin-bottom:20px">
			 ...
			 <c:if test="${fborderVO.fb_pay_status eq '已付款'&& fborderVO.fb_order_status eq '進行中'}">
			 	<button class="btn btn-primary btn-block" id="confirmReceive">確認收貨</button>
			 </c:if>
			 <c:if test="${fborderVO.fb_pay_status eq '未付款'}">
	...
	<div class="text-center col-md-12" style="margin-top:50px"><a href="<%=request.getContextPath()%>/FrontEnd/member/mem_fbOrder.jsp" class="btn btn-default">返回訂單列表</a></div>
	</div>	
		<!-- footer -->
	...
	$(function () {
	    $('#confirmCheck').on('click',function(){
	     if(window.confirm('確定已結帳?')){
	         ...
	         console.log(fb_order_no);
 	         console.log(fb_buyer_no);
	      $.ajax({
